...
A [MoneyStrategy](/reference/typescript-api/money/money-strategy#moneystrategy) that stores monetary values as a `bigint` type in the database, which
...
multiplying the unit price, rather than before (as is the case with the [DefaultMoneyStrategy](/reference/typescript-api/money/default-money-strategy#defaultmoneystrategy).
...
```
* Implements: [`MoneyStrategy`](/reference/typescript-api/money/money-strategy#moneystrategy)

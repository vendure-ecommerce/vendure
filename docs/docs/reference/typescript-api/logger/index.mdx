...
The actual implementation - where the logs are written to - is defined by the <DocsLink href="/reference/typescript-api/logger/vendure-logger#vendurelogger">VendureLogger</DocsLink>
instance configured in the <DocsLink href="/reference/typescript-api/configuration/vendure-config#vendureconfig">VendureConfig</DocsLink>. By default, the <DocsLink href="/reference/typescript-api/logger/default-logger#defaultlogger">DefaultLogger</DocsLink> is used, which
logs to the console.

## Implementing a custom logger

A custom logger can be passed to the `logger` config option by creating a class which implements the
<DocsLink href="/reference/typescript-api/logger/vendure-logger#vendurelogger">VendureLogger</DocsLink> interface. For example, here is how you might go about implementing a logger which
logs to a file:

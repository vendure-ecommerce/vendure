...
The actual implementation - where the logs are written to - is defined by the [VendureLogger](/reference/typescript-api/logger/vendure-logger#vendurelogger)
instance configured in the [VendureConfig](/reference/typescript-api/configuration/vendure-config#vendureconfig). By default, the [DefaultLogger](/reference/typescript-api/logger/default-logger#defaultlogger) is used, which
logs to the console.

## Implementing a custom logger

A custom logger can be passed to the `logger` config option by creating a class which implements the
[VendureLogger](/reference/typescript-api/logger/vendure-logger#vendurelogger) interface. For example, here is how you might go about implementing a logger which
logs to a file:

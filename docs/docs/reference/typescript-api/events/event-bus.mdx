...
* when a Product is updated ([ProductEvent](/reference/typescript-api/events/event-types#productevent))
...
If the event contains a [RequestContext](/reference/typescript-api/request/request-context#requestcontext) object, the subscriber
...
should be offloaded to a background job using the [JobQueueService](/reference/typescript-api/job-queue/job-queue-service#jobqueueservice).

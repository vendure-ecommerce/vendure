...
and via the <DocsLink href="/reference/typescript-api/services/settings-store-service#settingsstoreservice">SettingsStoreService</DocsLink>. These differ from custom fields in that they are not associated
...
Provide a logging service which implements the <DocsLink href="/reference/typescript-api/logger/vendure-logger#vendurelogger">VendureLogger</DocsLink> interface.

...
from the `Order` entity's `totalQuantity` property, which uses <DocsLink href="/reference/typescript-api/data-access/calculated#calculated">Calculated</DocsLink> decorator and defines those relations as dependencies

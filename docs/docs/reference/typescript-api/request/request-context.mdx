...
It is a good practice to inject the RequestContext (using the <DocsLink href="/reference/typescript-api/request/ctx-decorator#ctx">Ctx</DocsLink> decorator) into
...
the active Channel, and so on. In addition, the <DocsLink href="/reference/typescript-api/data-access/transactional-connection#transactionalconnection">TransactionalConnection</DocsLink> relies on the
...
is to use the <DocsLink href="/reference/typescript-api/request/request-context-service#requestcontextservice">RequestContextService</DocsLink> `create()` method, which allows more control
...
process, e.g. to pass it to the Job Queue via the <DocsLink href="/reference/typescript-api/job-queue/job-queue-service#jobqueueservice">JobQueueService</DocsLink>.
### copy
...
The active <DocsLink href="/reference/typescript-api/entities/channel#channel">Channel</DocsLink> of this request.
### channelId

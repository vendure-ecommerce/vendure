...
by the [JobQueue](/reference/typescript-api/job-queue/#jobqueue)'s `add()` method. Note that the subscription capability is only supported
if the [JobQueueStrategy](/reference/typescript-api/job-queue/job-queue-strategy#jobqueuestrategy) implements the [InspectableJobQueueStrategy](/reference/typescript-api/job-queue/inspectable-job-queue-strategy#inspectablejobqueuestrategy) interface (e.g.
the [SqlJobQueueStrategy](/reference/typescript-api/job-queue/sql-job-queue-strategy#sqljobqueuestrategy) does support this).
...
```
* Extends: [`Job`](/reference/typescript-api/job-queue/job#job)`<T>`
...
Polling interval, timeout and other options may be configured with an options arguments [JobUpdateOptions](/reference/typescript-api/job-queue/types#jobupdateoptions).

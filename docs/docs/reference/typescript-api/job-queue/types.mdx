...
Job update status as returned from the [SubscribableJob](/reference/typescript-api/job-queue/subscribable-job#subscribablejob)'s `update()` method.
...
Job update options, that you can specify by calling [SubscribableJob](/reference/typescript-api/job-queue/subscribable-job#subscribablejob) `updates` method.
...
Used to configure a new [JobQueue](/reference/typescript-api/job-queue/#jobqueue) instance.
...
A JSON-serializable data type which provides a [Job](/reference/typescript-api/job-queue/job#job)with the data it needs to be processed.
...
Used to instantiate a new [Job](/reference/typescript-api/job-queue/job#job)

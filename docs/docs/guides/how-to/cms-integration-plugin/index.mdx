...
The service also includes `syncVariantToCms()` and `syncCollectionToCms()` methods that follow the same pattern as the product sync shown above.
...
This section provides a high-level overview. For a complete, production-ready implementation, see the Strapi integration example:
https://github.com/vendure-ecommerce/examples/tree/publish/examples/strapi-cms-integration
...
private async updateDocumentFromProduct(product: Product, defaultLanguageCode: LanguageCode, productSlug?: string | null) {
  const t = product.translations.find(tr => tr.languageCode === defaultLanguageCode);
  if (!t?.slug) return;
  const existing = await this.findDocumentBySlug('vendure-product', t.slug);
  if (!existing) return this.createDocumentFromProduct(product, defaultLanguageCode, productSlug);
  const data = await this.transformProductData(product, defaultLanguageCode, productSlug);
  if (!data) return;
  await this.makePayloadRequest({ method: 'PATCH', endpoint: `vendure-product/${existing.id}`, data });
  Logger.info(`Updated product ${product.id} (Payload ID: ${existing.id})`);
}
...
private async deleteDocumentFromProduct(product: Product, defaultLanguageCode: LanguageCode) {
  ...
  const existing = await this.findDocumentBySlug('vendure-product', t.slug);
  if (!existing) return;
  await this.makePayloadRequest({ method: 'DELETE', endpoint: `vendure-product/${existing.id}` });
  Logger.info(`Deleted product ${product.id} (Payload ID: ${existing.id})`);
}
```

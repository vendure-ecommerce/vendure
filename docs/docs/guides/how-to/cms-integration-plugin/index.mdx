...
The service follows Vendure's dependency injection pattern and exposes sync methods for products, variants, and collections, delegating to CRUD helpers against Strapi.

```ts title="src/plugins/cms/services/strapi.service.ts"
@Injectable()
export class StrapiService {
    // Prefer configured base URL; default to local Strapi
...
export const vendureProduct = defineType({
    ...
    // define the entity types here too...
});
```
...
export class SanityService {
    private get apiBase(): string {
        // Sanity API base with version; dataset is provided per endpoint
    ...
    // POST query to avoid URL length and simplify params
...
private async transformProductData(product: Product, defaultLanguageCode: LanguageCode, productSlug?: string | null) {
  ...
  // Build relationships: variants of this product as Sanity references
...
export class PayloadService {
    // Base URL from configuration; default for local development
...
private getPayloadHeaders(): Record<string, string> {
  ...
  if (this.options.payloadApiKey) headers.Authorization = `Bearer ${this.options.payloadApiKey}`;
  return headers;
}

private async makePayloadRequest({ method, endpoint, data }: {
  method: 'GET' | 'POST' | 'PATCH' | 'DELETE'; endpoint: string; data?: any;
}) {
...
private async findDocumentsBySlugs(collectionSlug: string, slugs: string[]): Promise<Map<string, any>> {
  ...
  const slugsParam = slugs.map(s => encodeURIComponent(s)).join(',');
  const res = await this.makePayloadRequest({ method: 'GET', endpoint: `${collectionSlug}?where[slug][in]=${slugsParam}` });
  for (const doc of res.docs ?? []) if (doc?.slug) map.set(doc.slug, doc);
  return map;
}
```

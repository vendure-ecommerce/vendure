...
export class SanityService implements OnApplicationBootstrap {
  ...
  // Called when the application starts - Sanity doesn't require content type setup
  async onApplicationBootstrap() {
  ...
  private transformProductData(product) {
    return {
      ...
      title: product.translations[0]?.name, // Product name from first translation
      slug: { current: product.translations[0]?.slug }, // Sanity slug format

...
export class StoryblokService implements OnApplicationBootstrap {
    ...
    private async transformProductData(product, defaultLanguageCode, productSlug?) {
        const defaultTranslation = this.getTranslationByLanguage(product.translations, defaultLanguageCode);

        if (!defaultTranslation) {
            return undefined;
        }

        // Find all variant story UUIDs for this product
        ...
        return {
            story: {
                ...
                content: {
                    ...
                    variants: variantStoryIds, // Array of story UUIDs
                },
            },
            publish: 1, // Auto-publish the story
        };
    }
    ...
    private async ensureStoryContentTypesExists() {
        // Check if Vendure components exist in Storyblok
        const contentCheck = await this.checkContentTypes();

        // Create missing components with proper schema
    ...
    private async enforceRateLimit() {
        // Storyblok allows max 5 requests per second
        await new Promise(resolve => setTimeout(resolve, this.rateLimitDelay));
    }

    // Additional methods: batch story lookups, relationship management,
    // component creation, variant and collection sync operations, etc.
}
```

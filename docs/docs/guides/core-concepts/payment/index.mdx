---
title: 'Payment'
metaTitle: "Payment Integration and Workflows in Vendure"
metaDescription: "Understand payment workflows in Vendure, including authorization, settlement, single-step and two-step flows."
---
...
A [`PaymentMethod`](/reference/typescript-api/entities/payment-method/) can be created via the Dashboard (_Settings_ -> _Payment methods_, then _Create new payment method_) or via the Admin API `createPaymentMethod` mutation.
...
Once the active Order has been transitioned to the `ArrangingPayment` state (see the [Order guide](/core-concepts/orders/)), the available payment methods can be queried using the [`eligiblePaymentMethods` query](/reference/graphql-api/shop/queries#eligiblepaymentmethods).

### Add payment to order

One or more Payments are created by executing the [`addPaymentToOrder` mutation](/reference/graphql-api/shop/mutations#addpaymenttoorder). This mutation has a required `method` input field, which must match the `code` of an eligible `PaymentMethod`.

This mutation internally invokes the [PaymentMethodHandler's `createPayment()` function](/reference/typescript-api/payment/payment-method-config-options/#createpayment), which returns a [CreatePaymentResult object](/reference/typescript-api/payment/payment-method-types#createpaymentfn) used to create a new [Payment](/reference/typescript-api/entities/payment). If the Payment amount equals the order total, the Order is transitioned to either the `PaymentAuthorized` or `PaymentSettled` state and the customer checkout flow is complete.
...
For details on creating custom payment integrations and custom payment flows, see the [Developer Guide](/developer-guide/strategies-configurable-operations/).

## Code Examples

### Creating a PaymentMethodHandler
...
### Querying eligible payment methods

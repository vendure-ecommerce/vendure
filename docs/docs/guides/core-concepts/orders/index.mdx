---
title: 'Orders'
metaTitle: "Order Lifecycle and State Machine in Vendure"
metaDescription: "Vendure's order process from cart to delivery, powered by a finite state machine. Covers default states and transitions."
---
...
The current state of an order is available via the `state` field on the `Order` type. The next possible states can be queried via the [`nextOrderStates`](/reference/graphql-api/shop/queries/#nextorderstates) query.
...
[Order Interceptors](/reference/typescript-api/orders/order-interceptor/) allow you to intercept operations that add, modify, or remove order lines. Example use-cases include:
...
For details on customizing the order process, see the [Developer Guide](/developer-guide/strategies-configurable-operations/).

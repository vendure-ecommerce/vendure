...
export class ProductService {
  ...
  async getProductsForUser(ctx: RequestContext, userId: ID): Promise<Product[]> {
  ...
  async create(ctx: RequestContext, input: CreateProductInput): Promise<Translated<Product>> {
    const currentUser = await this.userService.getBareUserById(ctx, <number>ctx.activeUserId);
    if (!currentUser) throw new EntityNotFoundError('User', <number>ctx.activeUserId);
  ...
  async addOptionGroupToProduct(
    ...
    const optionGroup = await this.connection.getRepository(ctx, ProductOptionGroup).findOne(optionGroupId);
    if (!optionGroup) {

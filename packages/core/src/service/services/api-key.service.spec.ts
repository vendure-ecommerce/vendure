...
describe('ApiKeyService', () => {
    ...
    const admin = new Administrator({ id: 'admin-1' as any });
    const apiKeyRepo = {
        save: vi.fn(),
        findOne: vi.fn(),
        find: vi.fn(),
    ...
    beforeEach(() => {
        ...
        connection = {
            ...
            getEntityOrThrow: vi.fn(async (_ctx: any, entity: any, id: any) => {
                ...
                if (entity === ApiKey) {
                    return new ApiKey({ id, administrator: admin, name: 'Key A', status: 'active' } as any);
                }
                return {} as any;
            }),
        } as any;
    ...
    it('create(): creates key and returns raw secret; enforces unique active name', async () => {
        ...
        const result = await service.create({} as any, {
            administratorId: admin.id as any,
            name: 'Key A',
        });
        expect(result.rawKey).toMatch(/^vk_test_/);
        ...
        await expect(
            service.create({} as any, { administratorId: admin.id as any, name: 'Key A' }),
        ).rejects.toBeTruthy();
    });
